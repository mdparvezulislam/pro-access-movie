...
export default async function SearchPage({
  ...
  let results = query ? await searchPublishedContent({ query, limit: 24 }) : [];
    ...
    <div className="flex flex-col min-h-screen bg-background text-text-primary">
      ...
      <main className="flex-1 mx-auto max-w-7xl w-full px-4 sm:px-6 lg:px-8 py-6 sm:py-10 space-y-6 sm:space-y-8">
        ...
        <div className="max-w-2xl mx-auto text-center space-y-4">
          <h1 className="text-2xl sm:text-3xl font-black text-text-primary tracking-tight flex items-center justify-center gap-2">
            <Search className="h-6 w-6 sm:h-7 sm:w-7 text-red-500" />
          ...
          <form action="/search" method="GET" className="relative w-full">
            ...
            <Input
              ...
              className="pl-12 h-12 text-sm bg-surface-raised border-border/80 focus:border-red-600 rounded-2xl shadow-lg"
            />
          </form>
          ...
          <div className="flex items-center justify-center gap-1.5 flex-wrap pt-2">
            {DEMO_GENRES.map((g) => (
              <Link
                ...
                className="px-3 py-1 rounded-xl bg-surface-raised border border-border/80 text-xs text-text-secondary hover:text-text-primary hover:border-red-500/40 transition-colors"
              >
                {g.name_bn || g.name}
              </Link>
            ))}
        ...
        {query ? (
          <div className="space-y-4 pt-4 border-t border-border/60">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-text-muted font-mono font-bold">{results.length} items found</span>
            </div>
            ...
            {results.length > 0 ? (
              ...
              <div className="p-12 text-center rounded-3xl bg-surface-raised border border-border/80 space-y-3 shadow-xl">
          ...
          <div className="space-y-4 pt-4 border-t border-border/60">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-base sm:text-lg font-bold text-text-primary">Recommended Searches</h2>
            </div>
