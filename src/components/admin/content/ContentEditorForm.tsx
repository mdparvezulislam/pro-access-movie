...
export function ContentEditorForm({ id, type, initialData }: ContentEditorFormProps) {
  ...
  // 5 Primary Sections: content | media | sources | discovery | publish
  const [activeTab, setActiveTab] = useState<"content" | "media" | "sources" | "discovery" | "publish">("content");
  ...
  useEffect(() => {
    async function loadTaxonomies() {
      try {
        ...
        console.error("Failed to load content taxonomy relationships:", err);
      }
    }
    loadTaxonomies();
  }, [type, id]);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      router.push(`/admin/${type === "movie" ? "movies" : "series"}`);
    } catch (err: unknown) {
    ...
    <div className="w-full space-y-6 pb-24">
      ...
      <div className="p-4 sm:p-5 rounded-2xl bg-surface-base border border-border shadow-xl flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <Button
            ...
            variant="ghost"
            size="sm"
            onClick={() => router.push(`/admin/${type === "movie" ? "movies" : "series"}`)}
          ...
          <div className="space-y-1 min-w-0">
            ...
            <div className="flex items-center gap-2 text-xs text-text-muted flex-wrap">
              <span className="capitalize font-bold text-purple-400">{type}</span>
              <span>•</span>
      ...
      <div className="flex items-center gap-1 p-1.5 rounded-2xl bg-surface-base border border-border overflow-x-auto shadow-md">
        {[
          ...
          { id: "sources", label: "SOURCES", icon: Video },
      ...
      {activeTab === "content" && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="p-6 rounded-2xl bg-surface-base border border-border space-y-4 shadow-xl">
              <h2 className="text-sm font-bold text-text-primary flex items-center gap-2">
                <FileText className="h-4 w-4 text-purple-400" /> Basic Metadata
              </h2>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-text-secondary">Title (English) *</label>
                  <Input
                    value={title}
                    onChange={(e) => {
              ...
              {/* Descriptions */}
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-text-secondary">Overview Description (English)</label>
                <textarea
                  rows={4}
                  value={description}
                  onChange={(e) => {
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-text-secondary">Bengali Description (বাংলা বিবরণ)</label>
                <textarea
                  rows={4}
                  value={descriptionBn}
                  onChange={(e) => {
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    placeholder="e.g. Mejbaur Rahman Sumon"
                    className="h-10 text-xs bg-surface-raised border-border"
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    placeholder="e.g. Chanchal Chowdhury, Nazifa Tushi"
                    className="h-10 text-xs bg-surface-raised border-border"
                  />
                </div>
              </div>
          ...
          <div className="space-y-6">
            <div className="p-6 rounded-2xl bg-surface-base border border-border space-y-4 shadow-xl">
              <h2 className="text-sm font-bold text-text-primary flex items-center gap-2">
                <Sliders className="h-4 w-4 text-purple-400" /> Content Specifications
              </h2>
      ...
      {activeTab === "media" && (
        <div className="p-6 rounded-2xl bg-surface-base border border-border space-y-6 shadow-xl">
          <h2 className="text-sm font-bold text-text-primary flex items-center gap-2">
            <ImageIcon className="h-4 w-4 text-purple-400" /> Centralized Media Assets
          </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <MediaPicker
                ...
                aspectRatio="poster"
                folder={type === "movie" ? "movie" : "series"}
              />
            </div>

            <div className="space-y-2">
              <MediaPicker
                label="Backdrop Image (Landscape 16:9)"
                value={backdropPath}
                onChange={(url) => {
                  setBackdropPath(url);
                  setIsDirty(true);
      ...
      {activeTab === "sources" && (
        <div className="p-6 rounded-2xl bg-surface-base border border-border space-y-6 shadow-xl">
          <h2 className="text-sm font-bold text-text-primary flex items-center gap-2">
            <Video className="h-4 w-4 text-purple-400" /> Streaming & Download Sources
          </h2>
          <MediaSourceStudio contentId={id} contentType={type} />
        </div>
      )}

      {/* TAB 4: DISCOVERY & TAXONOMY */}
      {activeTab === "discovery" && (
        <div className="space-y-6">
          {/* Taxonomy Section */}
          <div className="p-6 rounded-2xl bg-surface-base border border-border space-y-6 shadow-xl">
            <h2 className="text-sm font-bold text-text-primary flex items-center gap-2">
              <Layers className="h-4 w-4 text-purple-400" /> Content Taxonomies & Collections
            </h2>
              ...
              <div className="space-y-4 pt-4 border-t border-border text-xs">
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    placeholder="e.g. Watch Hawa Movie Online Free in HD"
                    className="h-10 text-xs bg-surface-raised border-border"
                  />
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="font-semibold text-text-secondary">SEO Keywords (Comma Separated)</label>
                    <Input
                      value={seoKeywords}
                      onChange={(e) => {
                      ...
                      placeholder="hawa, bangla movie, chanchal chowdhury"
                      className="h-10 text-xs bg-surface-raised border-border"
                    />
                  </div>

                  <div className="space-y-1.5">
                    <label className="font-semibold text-text-secondary">Internal Search Tags</label>
                    <Input
                      ...
                      placeholder="hawa full movie hd, bangla mystery"
                      className="h-10 text-xs bg-surface-raised border-border"
                    />
                  </div>
                </div>
      ...
      {/* TAB 5: PUBLISH & DESTRUCTIVE ACTIONS */}
      {activeTab === "publish" && (
        <div className="space-y-6">
          <div className="p-6 rounded-2xl bg-surface-base border border-border space-y-4 shadow-xl">
            <h2 className="text-sm font-bold text-text-primary flex items-center gap-2">
              <Shield className="h-4 w-4 text-purple-400" /> Publishing Controls
            </h2>
          ...
          <div className="p-6 rounded-2xl bg-surface-base border border-red-500/30 space-y-4 shadow-xl">
            ...
            <p className="text-xs text-text-muted">
              Permanently delete this content record along with attached sources and metadata.
            </p>
            <Button
              type="button"
              variant="destructive"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-2xl bg-surface-base border border-red-500/30 p-6 space-y-4 shadow-2xl">
            ...
            <p className="text-xs text-text-muted leading-relaxed">
              This action cannot be undone. All associated playback links and streaming sources will be permanently removed.
            </p>
